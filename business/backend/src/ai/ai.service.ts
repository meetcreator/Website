...
`.trim();
            ...
            if (lowerMsg.includes('revenue') || lowerMsg.includes('sales')) return "📊 [Demo] Monthly revenue is $842.9k — up 14.2%. Enterprise tier contributes 68% of ARR.";
            if (lowerMsg.includes('stock') || lowerMsg.includes('inventory')) return "📦 [Demo] 2 SKUs critically low: 'Executive Suite A1' at 12 units (min 20).";
            if (lowerMsg.includes('profit') || lowerMsg.includes('burn')) return "💰 [Demo] Operating profit: $124.5k this month (+5.2%). Burn rate: -$12.4k/mo.";
            if (lowerMsg.includes('churn') || lowerMsg.includes('risk')) return "⚠️ [Demo] Churn risk at 14.2%. 'At Risk' cohort shows 32% cart abandonment.";
        ...
        try {
            ...
            const topProductsText = products.slice(0, 5)
                .map((p: any, i: number) => `${i + 1}. ${p.name} — ${p.orders?.length ?? 0} orders`)
                .join('\n') || 'No product data yet';

            const lowStockText = lowStockItems.length > 0
                ? lowStockItems.map((inv: any) => `- ${inv.product?.name ?? 'Unknown'}: ${inv.quantity} units (min: ${inv.minThreshold})`).join('\n')
...
`.trim();
            ...
            // Fallback: keyword-based using real data
            const lowerMsg = message.toLowerCase();
